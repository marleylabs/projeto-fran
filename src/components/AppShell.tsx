...
export function AppShell({children}:{children:ReactNode}){
  ...
  const navigation=<><div className="app-brand"><span className="app-brand-mark">GA</span><div><strong>Gestão Administrativa</strong><span>Plataforma corporativa</span></div></div><nav className="app-nav" aria-label="Módulos da plataforma"><p className="app-nav-label">Navegação</p>{NAVIGATION_MODULES.map(module=>{const active=isActive(pathname,module.matchPaths);const Icon=glyphs[module.id];return module.href?<Link key={module.id} href={module.href} onClick={()=>setOpen(false)} className={clsx("app-nav-item",active&&"is-active")} aria-current={active?"page":undefined}><span aria-hidden="true">{Icon&&<Icon size={16}/>}</span>{module.label}</Link>:<span key={module.id} className="app-nav-item is-disabled" title="Módulo planejado"><span aria-hidden="true">{Icon&&<Icon size={16}/>}</span>{module.label}<small>Em breve</small></span>})}</nav><footer className="app-user"><div className="app-avatar">{initials(user?.name||user?.email||"")}</div><div className="min-w-0 flex-1"><strong className="block truncate">{user?.name||"Usuário"}</strong><span className="block truncate">{user?.roles?.[0]||user?.email||"Sessão ativa"}</span></div><button type="button" onClick={logout} aria-label="Sair da aplicação" title="Sair"><LogOut size={16}/></button></footer></>;
  return <div className="app-shell"><aside className="app-sidebar">{navigation}</aside><header className="app-mobile-header"><button type="button" onClick={()=>setOpen(true)} aria-label="Abrir navegação"><Menu size={18}/></button><strong>Gestão Administrativa</strong></header>{open&&<div className="app-drawer" role="dialog" aria-modal="true" aria-label="Navegação"><button className="app-drawer-backdrop" onClick={()=>setOpen(false)} aria-label="Fechar navegação"/><aside>{navigation}</aside></div>}<div className="app-content">{children}</div></div>;
}
